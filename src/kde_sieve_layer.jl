struct KDESieveLayer{T₁ <: Number}
    weights::Matrix{T₁}
    comparators::Vector{KDEComparator}
    next::Union{KDESieveLayer{T₁},Nothing}

    function KDESieveLayer{T₁}(samples::T₂,
                               targets::T₃,
                               n_projections::Int,
                               remaining_depth::Int,
        ...
        if remaining_depth == 0 || length(t₀) < min_split ||
            ...
            next = KDESieveLayer{T₁}(X₀, t₀, L, remaining_depth - 1, min_split)
        end
